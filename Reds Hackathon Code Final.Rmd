...
map(pitcher_management$ab_id, function(x) {
  savant %>% 
    ungroup() %>%
    filter(ab_id == x) %>%
    slice_max(pitch_number) %>% 
    select(delta_run_exp, estimated_woba_using_speedangle, bb_type, 
           events, ab_id)
}, .progress = TRUE) %>% 
  bind_rows() -> d_run_exp
...
pitcher_management <- pitcher_management %>%
  select(-ab_id)
